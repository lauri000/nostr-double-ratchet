...
describe("DeviceRecord", () => {
  ...
  it("only forwards device rumors when authorization passes", () => {
    ...
    record.installSession(session, true, { persist: false })
  ...
  it("allows events from the active session even before authorization", () => {
    const { deps, userHooks, setAuthorized } = createDeps({
      ownerPubkey: "owner-pubkey",
      ourDeviceId: "our-device-id",
    })
    const record = new DeviceRecord("peer-device-id", deps)
    const { session, emit } = createFakeSession("session-active")
    const rumor = makeRumor("rumor-active", "hello")
