...
async fn test_publish_fetch_and_message_both_ways() {
    ...
    let published_url = publish["data"]["url"]
        .as_str()
        .expect("Expected invite URL");
    ...
    assert!(
        !events.is_empty(),
        "Expected at least one invite event from relay"
    );
    ...
    assert!(
        !stored_invites.is_empty(),
        "Alice should have a stored invite"
    );
    ...
    let bob_msg_event = bob_session
        .send("hello from bob".to_string())
        .expect("Bob failed to send message");
    let decrypted = alice_session
        .receive(&bob_msg_event)
    ...
    let alice_msg_event = alice_session
        .send("hello from alice".to_string())
        .expect("Alice failed to send message");
    let decrypted = bob_session
        .receive(&alice_msg_event)
    ...
    let bob_msg2 = bob_session
        .send("second message from bob".to_string())
