...
fn establish_active_pair(
    ...
    let first = alice_manager.prepare_send_text(
        &mut alice_send_ctx,
        bob.owner_pubkey,
        "bootstrap".into(),
    )?;
...
fn post_restore_delayed_invite_response_establishes_sendable_session_once() -> Result<()> {
    ...
    let prepared =
        restored.prepare_send_text(&mut send_ctx, bob.owner_pubkey, "alice-reply".into())?;
    assert_eq!(prepared.deliveries.len(), 1);
    assert!(prepared.invite_responses.is_empty());

    let event =
        nostr_double_ratchet::codec::nostr::direct_message_event(&prepared.deliveries[0].envelope)?;
    let received_reply =
        support::receive_event(&mut bob_session, &mut support::context(127, 127), &event)?;
...
fn post_restore_receive_then_reply_uses_existing_session_without_invite_response() -> Result<()> {
    ...
    let reply = restored.prepare_send_text(
        &mut reply_ctx,
        bob.owner_pubkey,
        "reply-after-restore".into(),
    )?;
...
fn peer_device_added_after_conversation_bootstraps_only_new_device() -> Result<()> {
    ...
    let prepared = alice_manager.prepare_send_text(
        &mut send_ctx,
        bob1.owner_pubkey,
        "fanout-new-device".into(),
    )?;
...
fn local_sibling_added_after_restore_receives_self_fanout_without_rebootstrapping_peer_devices(
    ...
    let prepared =
        restored.prepare_send_text(&mut send_ctx, bob.owner_pubkey, "self-fanout".into())?;
    assert_eq!(prepared.deliveries.len(), 2);
    assert_eq!(prepared.invite_responses.len(), 1);
...
fn equal_timestamp_appkeys_merge_after_restore_unlocks_disjoint_devices() -> Result<()> {
    ...
    restored.observe_device_invite(bob1.owner_pubkey, bob2_local_invite.clone())?;
    ...
    let prepared = restored.prepare_send_text(&mut send_ctx, bob1.owner_pubkey, "merged".into())?;
    assert_eq!(prepared.deliveries.len(), 2);
    ...
    assert_eq!(
        received_contents(&[bob1_received, bob2_received]),
        vec!["merged", "merged"]
    );
    Ok(())
}
...
fn multi_device_transcript_survives_roundtrip_of_both_managers() -> Result<()> {
    ...
    alice1_manager.observe_device_invite(alice1.owner_pubkey, alice2_invite.clone())?;
    bob1_manager.observe_device_invite(bob1.owner_pubkey, bob2_invite.clone())?;

    let mut alice_send_ctx = support::context(238, 238);
    let first =
        alice1_manager.prepare_send_text(&mut alice_send_ctx, bob1.owner_pubkey, "m1".into())?;
    ...
    let mut alice1_manager =
        restore_manager(&alice1_manager.snapshot(), alice1.secret_key, 60 * 60)?;
    let mut bob1_manager = restore_manager(&bob1_manager.snapshot(), bob1.secret_key, 60 * 60)?;
    let mut alice2_manager =
        restore_manager(&alice2_manager.snapshot(), alice2.secret_key, 60 * 60)?;
    ...
    let second =
        bob1_manager.prepare_send_text(&mut bob_send_ctx, alice1.owner_pubkey, "m2".into())?;
    for response in &second.invite_responses {
        let recipient = response.recipient;
        if recipient == alice1_invite.inviter_ephemeral_public_key {
            manager_observe_invite_response(
                &mut alice1_manager,
                &mut support::context(246, 246),
                response,
            )?;
        } else if recipient == alice2_invite.inviter_ephemeral_public_key {
            manager_observe_invite_response(
                &mut alice2_manager,
                &mut support::context(247, 247),
                response,
            )?;
        } else if recipient == bob2_invite.inviter_ephemeral_public_key {
            manager_observe_invite_response(
                &mut bob2_manager,
                &mut support::context(248, 248),
                response,
            )?;
        }
    }
