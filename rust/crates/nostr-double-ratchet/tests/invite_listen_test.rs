...
#[test]
fn test_owner_claim_verification_requires_app_keys_for_multi_device() -> Result<()> {
    let alice_keys = Keys::generate();
    let alice_pk = alice_keys.public_key();
    let alice_sk = alice_keys.secret_key().to_secret_bytes();
    let invite = Invite::create_new(alice_pk, Some("alice-device".to_string()), None)?;
...
#[test]
fn test_owner_claim_verification_allows_single_device_without_app_keys() -> Result<()> {
    let alice_keys = Keys::generate();
    let alice_pk = alice_keys.public_key();
    let alice_sk = alice_keys.secret_key().to_secret_bytes();
    let invite = Invite::create_new(alice_pk, Some("alice-device".to_string()), None)?;
