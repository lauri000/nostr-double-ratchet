use crate::{
    pubsub::build_filter, pubsub::NostrPubSub, Error, EventCallback, Result, Session,
    SessionInitInput, SessionReceiveInput, SessionReceiveResult, SessionSendInput, SessionState,
...
impl SessionActor {
    ...
    pub fn send_event(&mut self, event: UnsignedEvent) -> Result<nostr::Event> {
        let now = std::time::SystemTime::now()
            ...
            .unwrap();
        let output = self.as_session().send_event(SessionSendInput {
            inner_event: event,
            now_secs: now.as_secs(),
            now_ms: now.as_millis() as u64,
        })?;
        self.state = output.next.state;
    ...
    pub fn receive(&mut self, event: &nostr::Event) -> Result<Option<String>> {
        ...
        match self.as_session().receive_event(SessionReceiveInput {
...
impl From<Session> for SessionActor {
    fn from(session: Session) -> Self {
        Self::from_session(session, "session".to_string())
    }
}

impl From<SessionActor> for Session {
    fn from(session: SessionActor) -> Self {
