...
use nostr::{
    Event, EventBuilder, EventId, Keys, PublicKey, SecretKey, Tag, Timestamp, UnsignedEvent,
};
...
impl Session {
    ...
    pub fn init(input: SessionInitInput) -> Result<Self> {
        let our_keys = Keys::new(SecretKey::from_slice(
            &input.our_ephemeral_nostr_private_key,
        )?);
        ...
        let (root_key, sending_chain_key) = if input.is_initiator {
            ...
            let conversation_key = nip44::v2::ConversationKey::derive(
                &next_sk,
                &input.their_ephemeral_nostr_public_key,
            );
    ...
    pub fn receive_event(&self, input: SessionReceiveInput) -> SessionReceiveResult {
        ...
        let mut next = self.clone();
        let outcome =
            (|| -> Result<(Session, String, Option<UnsignedEvent>, SessionReceiveMeta)> {
                ...
                let (header, should_ratchet) =
                    decrypt_header(&next.state, &encrypted_header, &sender)?;
                ...
                if header.next_public_key != their_next_hex {
                    next.state.their_current_nostr_public_key =
                ...
                if should_ratchet {
                    if next.state.receiving_chain_key.is_some() {
                        skip_message_keys(&mut next.state, header.previous_chain_length, &sender)?;
                    }
                    ratchet_step(&mut next.state, input.replacement_next_nostr_private_key)?;
                }

                let plaintext = ratchet_decrypt(
                    &mut next.state,
                    &header,
                    &input.outer_event.content,
                    &sender,
                )?;
...
fn decrypt_header(
    ...
    if let Some(current) = &state.our_current_nostr_key {
        ...
        if let Ok(decrypted) = nip44::decrypt(&current_secret, sender, encrypted_header) {
            let header = serde_json::from_str(&decrypted)
...
mod tests {
    ...
    fn session_send_returns_next_state_and_outputs() {
        ...
        assert_eq!(
            u32::from(output.outer_event.kind.as_u16()),
            MESSAGE_EVENT_KIND
        );
    ...
    fn session_receive_irrelevant_event_returns_not_for_this_session() {
        ...
        let unrelated =
            EventBuilder::new(nostr::Kind::Custom(MESSAGE_EVENT_KIND as u16), "ciphertext")
        ...
        assert!(matches!(
            result,
            SessionReceiveResult::NotForThisSession { .. }
        ));
    }
