...
impl Invite {
    ...
    pub fn accept(&self, input: InviteAcceptInput) -> Result<InviteAcceptResult> {
        ...
        let outer_event =
            EventBuilder::new(Kind::Custom(INVITE_RESPONSE_KIND as u16), envelope_content)
                .tag(
    ...
    pub fn process_response(
        &self,
        input: InviteProcessResponseInput,
        ...
        let outcome = (|| -> Result<(Invite, Session, InviteResponseMeta)> {
            let inviter_ephemeral_private_key = self.inviter_ephemeral_private_key.ok_or(
                Error::Invite("missing inviter ephemeral private key".to_string()),
            )?;
            let inviter_ephemeral_secret = SecretKey::from_slice(&inviter_ephemeral_private_key)?;
            let decrypted = nip44::decrypt(
                &inviter_ephemeral_secret,
                &input.event.pubkey,
                &input.event.content,
            )?;
            ...
            let inviter_identity_secret =
                SecretKey::from_slice(&input.inviter_identity_private_key)?;
    ...
    pub fn serialize(&self) -> Result<String> {
        let data = serde_json::json!({
            ...
            "ownerPublicKey": self.owner_public_key.as_ref().map(|pk| hex::encode(pk.to_bytes())),
        });
        Ok(data.to_string())
    }
...
mod tests {
    ...
    fn invite_accept_returns_session_and_response() {
        ...
        assert_eq!(
            u32::from(accepted.response_event.kind.as_u16()),
            INVITE_RESPONSE_KIND
        );
    ...
    fn process_response_accepts_legacy_raw_session_key_payload() {
        ...
        .unwrap();
        let event = EventBuilder::new(Kind::Custom(INVITE_RESPONSE_KIND as u16), envelope_content)
            .tag(
                Tag::parse(&[
                    "p".to_string(),
        ...
        match invite.process_response(InviteProcessResponseInput {
            event,
            inviter_identity_private_key: inviter_identity_sk.to_secret_bytes(),
            inviter_next_nostr_private_key: key_bytes(96),
        }) {
            InviteProcessResponseResult::Accepted { meta, session, .. } => {
                assert_eq!(meta.invitee_identity, invitee_identity_pk);
                assert_eq!(
                    session.state.their_next_nostr_public_key,
                    Some(invitee_session_keys.public_key())
                );
            }
            other => panic!("expected accepted invite response, got {other:?}"),
        }
